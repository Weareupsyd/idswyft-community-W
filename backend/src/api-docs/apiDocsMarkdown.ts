...
The status response also includes \`id_face_base64\` (padded headshot crop, ears/hair/chin preserved).
...
The endpoint requires either a valid \`X-API-Key\` or a session token scoped to the verification. Returns 404 if the face image is not yet available (front document still processing or face not detected).
